...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["fileToUpload3"]) && $_FILES["fileToUpload3"]["error"] == UPLOAD_ERR_OK) {
        // Get file details
        $fileName = $_FILES["fileToUpload3"]["name"];
        $fileTmpName = $_FILES["fileToUpload3"]["tmp_name"];
        ...
        if (move_uploaded_file($fileTmpName, $filePath)) {
            ...
            $checkSql = "SELECT * FROM waivers WHERE u_id = ?";
            ...
            if ($result->num_rows > 0) {
                ...
                $updateSql = "UPDATE waivers SET stud_name = ?, file_name = ?, file_type = ?, file_size = ?, file_path = ? WHERE u_id = ?";
                $updateStmt = $conn->prepare($updateSql);
                $updateStmt->bind_param("sssssi", $stud_name, $fileName, $fileType, $fileSize, $filePath, $u_id);
                if ($updateStmt->execute()) {
                    // Set session variable for success message
                    $_SESSION['success_message'] = "Waiver updated successfully.";
                } else {
                    echo "Error updating file: " . $updateStmt->error;
                }
            } else {
                // If file does not exist, insert a new record
                $insertSql = "INSERT INTO waivers (u_id, stud_name, file_name, file_type, file_size, file_path) VALUES (?, ?, ?, ?, ?, ?)";
                $insertStmt = $conn->prepare($insertSql);
                $insertStmt->bind_param("isssss", $u_id, $stud_name, $fileName, $fileType, $fileSize, $filePath);
                if ($insertStmt->execute()) {
                    // Set session variable for success message
                    $_SESSION['success_message'] = "Waiver uploaded successfully.";
                } else {
            ...
            $_SESSION['error_message'] = "ENo file uploaded or an error occurred.";
        }
    } else {
        // Set session variable for error message
