...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["fileToUpload1"]) && $_FILES["fileToUpload1"]["error"] == UPLOAD_ERR_OK) {
        // Get file details
        $fileName = $_FILES["fileToUpload1"]["name"];
        $fileTmpName = $_FILES["fileToUpload1"]["tmp_name"];
        ...
        if (move_uploaded_file($fileTmpName, $filePath)) {
            ...
            $checkSql = "SELECT * FROM resumes WHERE u_id = ?";
            ...
            if ($result->num_rows > 0) {
                ...
                $updateSql = "UPDATE resumes SET stud_name = ?, file_name = ?, file_type = ?, file_size = ?, file_path = ? WHERE u_id = ?";
                $updateStmt = $conn->prepare($updateSql);
                $updateStmt->bind_param("sssssi", $stud_name, $fileName, $fileType, $fileSize, $filePath, $u_id);
                if ($updateStmt->execute()) {
                    // Set session variable for success message
                    $_SESSION['success_message'] = "Resume updated successfully.";
                } else {
                    echo "Error updating file: " . $updateStmt->error;
                }
            } else {
                // If file does not exist, insert a new record
                $insertSql = "INSERT INTO resumes (u_id, stud_name, file_name, file_type, file_size, file_path) VALUES (?, ?, ?, ?, ?, ?)";
